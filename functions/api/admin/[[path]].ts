...
function allowedPath(path: string) {
  return Object.values(areas).flat().some(prefix => path === prefix || path.startsWith(prefix));
}
...
export const onRequestGet = async ({ request, env }: Context) => {
  try {
    ...
    const items = tree.tree.filter(item => item.type === 'blob' && prefixes.some(prefix => item.path === prefix || item.path.startsWith(prefix))).map(item => ({ path: item.path, label: item.path.split('/').pop() }));
